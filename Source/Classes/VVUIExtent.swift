...
protocol VVAdapterZoomCalculationable {
...
extension CGPoint: VVAdapterZoomCalculationable {
...
extension CGSize: VVAdapterZoomCalculationable {
...
extension CGRect: VVAdapterZoomCalculationable {
...
extension CGVector: VVAdapterZoomCalculationable {
...
extension UIOffset: VVAdapterZoomCalculationable {
...
extension UIEdgeInsets: VVAdapterZoomCalculationable {
    ...
    @IBInspectable private var zoomTitle: Bool {
        ...
        set {
            ...
            let titles = states.enumerated().compactMap {
                ...
                guard let t = attributedTitle(for: state) else {
                    return nil
                }
                return (i, t)
            }
    ...
    @IBInspectable private var zoomImage: Bool {
        ...
        set {
            ...
            let images = states.enumerated().compactMap {
                ...
                guard let v = image(for: state) else {
                    return nil
                }
                return (i, v)
            }
            ...
            let backgrounds = states.enumerated().compactMap {
                (i, state) -> (Int, UIImage)? in
                guard let v = backgroundImage(for: state) else {
                    return nil
                }
                return (i, v)
            }
...
fileprivate extension Array {
    ...
    func filtered<E: Equatable>(duplication closure: (Element) throws -> E) rethrows -> [Element] {
        return try reduce(into: [Element]()) { (result, e) in
            let contains = try result.contains {
                try closure($0) == closure(e)
            }
            result += contains ? [] : [e]
        }
    }
